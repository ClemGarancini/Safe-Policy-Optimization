...
def single_agent_args():
    ...
    if args.task in isaac_gym_map.keys():
        ...
        args = gymutil.parse_arguments(
            description="RL Policy", custom_parameters=issac_parameters
        )
...
def multi_agent_args(algo):
    ...
    custom_parameters = [
        ...
        },
        {
            "name": "--task",
            "type": str,
            "default": "Safety2x4AntVelocity-v0",
            "help": "The task to run",
        },
        {
            "name": "--agent-conf",
    ...
    cfg_train["log_dir"] = (
        "../runs/" + args.experiment + "/" + args.task + "/" + algo + "/" + relpath
    )
    cfg_env = {}
    if args.task in isaac_gym_map.keys():
        cfg_env_path = "marl_cfg/{}.yaml".format(isaac_gym_map[args.task])
        ...
        args.task in multi_agent_velocity_map.keys()
        or args.task in multi_agent_goal_tasks
    ):
        pass
    else:
        warn_task_name()
